...
const book = {
  Sheet1: [
    [10, 20], // A1, B1
    [30, BookEngine.createFormula("A1+B1+A2")] // A2, B2 (B2は A1(10) + B1(20) + A2(30) = 60 になる)
  ]
};
...
const book = {
  Sheet1: [
    [2, 3], // A1, B1
    [4, BookEngine.createFormula("PRODUCT(A1:B1)")] // A2, B2 -> 2 * 3 = 6
  ]
};
...
const book = {
  ...
  "集計シート": [
    [BookEngine.createFormula("'売上データ'!A1 * 1.1")] // A1 -> 1500 * 1.1 = 1650
  ]
};
...
engine.defineFunction("GET_RATE", async function(currency) {
  // 例: 外部APIを叩く想定
  // const res = await fetch(`https://api.example.com/rate/${currency}`);
  // return res.json();
...
* **引数**: `formulaStr` (String) - `A1+B2` や `SUM(A1:B5)` などの数式文字列。
...
* **例外**: 循環参照など、解決できない参照関係がある場合は `"解決できない参照先"` というエラーをスローします。
...
| `A1` | セル参照（列アルファベット + 行番号） | `B20` |
| `A1:B3` | 範囲参照（左上セル:右下セル） | `AVERAGE(A1:C5)` |
| `'Sheet'!A1` | シングルクォーテーション囲みでのシート跨ぎ参照 | `'2026年データ'!A1` |
